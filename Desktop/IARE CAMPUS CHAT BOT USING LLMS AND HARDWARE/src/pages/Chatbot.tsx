...
const Chatbot = () => {
  ...
  const [language, setLanguage] = useState(() => localStorage.getItem('chatLang') || 'en-US');
  ...
  const sendMessage = async (inputType: 'text' | 'voice') => {
    ...
    try {
      const langHint = language !== 'en-US' ? ` (User is communicating in ${language}. Respond in the same language.)` : '';
      const { data, error } = await supabase.functions.invoke('chat-grok', {
        body: { 
          message: userMessage + langHint,
      ...
      sendNotification({ title: 'IARE Assistant', body: aiResponse.slice(0, 100), tag: 'chat-response' });
    } catch (error: any) {
      ...
      <div className="container mx-auto px-4 py-6 h-[calc(100vh-180px)] flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <LanguageSelector value={language} onChange={(code) => { setLanguage(code); localStorage.setItem('chatLang', code); }} compact />
